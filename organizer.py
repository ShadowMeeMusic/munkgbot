...
# Мои конференции
@router.message(Command("my_conferences"))
async def my_conferences(message: types.Message):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for conf in conferences:
            ...
            text += f"Даты: {conf.date_start} — {conf.date_end}\n"
...
# Подтверждение участия (не зависит от бана)
...
# Приём скриншота оплаты (не зависит от бана)
...
async def receive_payment_screenshot(message: types.Message):
    async with AsyncSessionLocal() as session:
        ...
        if not apps:
            await message.answer("Вы не ожидаете оплату.")
            return
...
# Редактирование конференции — с усиленной валидацией дат
...
async def start_edit(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        await state.update_data(
            ...
            city=conf.city or "",
            date_start=conf.date_start or "",
            date_end=conf.date_end or "",
            fee=conf.fee,
...
async def edit_city(message: types.Message, state: FSMContext):
    ...
    await state.set_state(EditConference.date_start)
    await message.answer("4. Дата начала (ГГГГ-ММ-ДД):", reply_markup=get_cancel_keyboard())

@router.message(EditConference.date_start)
async def edit_date_start(message: types.Message, state: FSMContext):
    ...
    await state.set_state(EditConference.date_end)
    await message.answer("5. Дата окончания (ГГГГ-ММ-ДД):", reply_markup=get_cancel_keyboard())

@router.message(EditConference.date_end)
async def edit_date_end(message: types.Message, state: FSMContext):
    ...
    await message.answer("6. Оргвзнос (0 — бесплатно):", reply_markup=get_cancel_keyboard())
...
async def edit_fee(message: types.Message, state: FSMContext):
    ...
    await message.answer("7. QR-код (отправьте новое фото или напишите 'нет'):", reply_markup=get_cancel_keyboard())
